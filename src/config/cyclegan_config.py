...
def parse_args(phase):
    ...
    Help description for each configuration:
        ...
        data(str): Data type to choose. default: 'horse2zebra.zip'
        dataroot(str): Path of images (should have sub folders trainA, trainB, testA, testB, etc). Default: None.
        image_size(int): Input image_size. Default is 256.

        data_dir(str): The translation direction of CycleGAN. Default: 'testA'.
        outputs_dir(str): Models are saved here. Default: './outputs'.
    ...
    # data
    parser.add_argument('--data', type=str, default='horse2zebra.zip', \
                        choices=('apple2orange.zip', 'summer2winter_yosemite.zip', 'horse2zebra.zip',
                                 'monet2photo.zip', 'cezanne2photo.zip', 'ukiyoe2photo.zip', 'vangogh2photo.zip',
                                 'maps.zip', 'cityscapes.zip', 'facades.zip', 'iphone2dslr_flower.zip',
                                 'ae_photos.zip'))
